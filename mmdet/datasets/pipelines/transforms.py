...
class ShipTranslate(object):
    ...
    def __init__(self,
        ...
        if isinstance(direction, list):
            for direct in direction:
                assert direct in ('horizontal', 'vertical'), \
                    'direction should be "horizontal" or "vertical".'
        else:
            assert direction in ('horizontal', 'vertical'), \
                'direction should be "horizontal" or "vertical".'
        assert isinstance(min_translate_offset, (int, float)), \
            'The min_translate_offset must be type int or float.'
        ...
        self.min_translate_offset = min_translate_offset
    ...
    def _translate_bboxes(self, results, offset, direction):
    ...
    def __call__(self, results):
        ...
        if np.random.rand() > self.prob:
            return results
        offset = np.random.randint(self.min_translate_offset, self.max_translate_offset)
        offset = random_negative(offset, self.random_negative_prob)
        if isinstance(self.direction, list):
            direction = random.choice(self.direction)
        else:
            direction = self.direction
        self._translate_img(results, offset, direction)
        self._translate_bboxes(results, offset, direction)
        ...
        # of segmentation map.
        self._translate_seg(
            results, offset, direction, fill_val=self.seg_ignore_label)
...
class SpeakNoise:
    ...
    def __call__(self, results):
        ...
        img_ = np.array(blured_img, dtype=float)
        noise = np.random.normal(self.mean, self.var ** 0.5, img_.shape)
        noise = np.clip(noise, a_min=self.truncated[0], a_max=self.truncated[1])
        img_ = img_ * noise
        results['img'] = img_
        return results
            ...
            for bbox in bboxes:
                if random.uniform(0, 1) < self.prob:
                    ...
                    for _ in range(n_holes):
                        ...
                        results['img'][int(y1):int(y2), int(x1):int(x2), :] = self.fill_in
